...
/* General test to confirm that forward directional paths run and in proper order
 	(a)
 	 |
 	 v
 	(b)
...
TEST_F(ModuleTest, ForwardSimpleTree)
{
	...
	CallbackContinuer complete(&ErrorStorage);
	sTestError = ERR_UNKNOWN;

	SequenceIterator iter(a.GetHead());
	iter.Run(&mCtx, &complete);

	EXPECT_EQ("abc", mTestString);
	EXPECT_EQ(ERR_NONE, sTestError);
}

/* General test to confirm that backward directional paths run and in proper order
 	(a)
 	 ^
 	 |
 	(b)
...
/* General test to confirm that a tiny tree runs as expected
	 (a)
*/
...
/* General test to confirm that multi directional paths run and in proper order
	(a)
	 ^
	 |
	 v
...
/* Test to confirm that a cyclical tree functions properly
	(a)
	 |  ^
	 v   \
	(b)->(c)
...
/* General test to confirm that forking paths allow a dynamic decision
	 (a)
	  |
	 (b)
	/ | \
...
/* Test to confirm that multiple trees can be connected together
	(a)
	 |
	(a)
	 |
